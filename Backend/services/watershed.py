...
# ---------------------------------------------------------------------------
# Auto-watershed (disabled — heuristic didn't segment flakes reliably in testing)
#
# Approach: use the GMM ``flake_mask.png`` as a shape template — segment the
# target image into candidate blobs, then pick the one whose outline best matches
# the detected flake (cv2.matchShapes — invariant to scale, rotation and
# position), with an optional magnification-ratio size prior to break ties. This
# avoided the old "flake is centred" assumption but did not pick the correct
# flake often enough to be useful. Kept commented for future refinement; the
# user-guided run_watershed above remains the active path.
# ---------------------------------------------------------------------------
